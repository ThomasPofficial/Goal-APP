...
- Do not touch `StudentBrochureData` (Prisma model), `app/api/student/brochure-data/route.ts`, or `app/(dashboard)/profile/survey/` — the alumni "Your Outcomes" form, explicitly called out as untouched in the separate Survey-removal commit (`4431473`).
...
- `/school/roster`, `/school/alumni`, `/profile/survey` (the alumni "Your Outcomes" form) still load without errors — confirms the Brochure deletion didn't collaterally break anything outside its own subsystem.
