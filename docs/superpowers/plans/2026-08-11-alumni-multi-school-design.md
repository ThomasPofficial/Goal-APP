...
| `/api/school/roster/members` (POST), `/api/school/roster/import` | Adding an alumni row creates/links an `AlumniSchool` row (upsert on `[profileId, schoolId]`) instead of overwriting `schoolId` — linking School B no longer evicts them from School A. |
| `/api/school/roster/members/[userId]` (DELETE) | For alumni, deletes only the `AlumniSchool` row for that specific school. For non-alumni, unchanged (`schoolId: null`). |
...
- **`AlumniProfileEditor.tsx`** — new "Schools" section: lists currently
  linked schools with a remove action, plus search/select to add another
  (search over SCHOOL-role users by name). This is the actual feature surface
  a user interacts with — everything else exists to support it.
